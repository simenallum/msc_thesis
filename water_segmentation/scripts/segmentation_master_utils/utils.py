...
def find_safe_areas(image, x):
    # Define a sliding window of size x
    window = np.ones((x, x), dtype=np.uint8)

    binary_img = np.where(image > 0, 0, 1)
    center = np.array(binary_img.shape) / 2

    # Crop a subarray centered at the image center and of size x times x
    center_window = binary_img[int(center[0]-x/2):int(center[0]+x/2), int(center[1]-x/2):int(center[1]+x/2)]

    # Check if the center window is safe
    if np.sum(np.multiply(center_window, window)) == x**2:
        return np.array([center])

    # Set the stride
    stride = 10

    # Apply the kernel to the image with the specified stride
    ...
    for i in range(len(windows)):
        for j in range(len(windows[i])):
            ...
            if np.sum(res) == x**2:
                safe_loc.append([(i*stride)+(x//2), (j*stride)+(x//2)])
...
if __name__ == '__main__':
    ...
    sorted = find_safe_areas(img, px_safe_dist)
    plt.imshow(img)
